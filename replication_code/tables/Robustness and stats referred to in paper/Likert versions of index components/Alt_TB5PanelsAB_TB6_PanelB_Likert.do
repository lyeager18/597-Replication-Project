...
Purpose: 	Robustness	-	Alternate Table B5, Panels A & B and Table B6, Panel 
							B; Ability to leave the house, Ability to make 
							purchases; multiple hypothesis testing; multiple HTE;
							Likert version
								  
								  
TABLE FOOTNOTES:						
Table B5: Alternate version of Table B5, Panels A & B, where we use the full likert 
scale to generate the index outcomes. Variations in sample size are due to drop-off 
from telephone survey; order of survey modules was randomized. Outcomes are defined 
as described in Table 1, except that this version does not transform responses 
into binary indicators for above median response. All estimates include individual 
and household controls: age (above median dummy), education level (less than a 
high school degree), marital status (indicators for married, never-married, and 
...
except for the interaction control. As such, some Ns are lower relative to Table 
1. 10 respondents are missing values for education level at baseline, with some 
overlap in respondents who are also missing values for outcomes. Four respondents 
are missing values marital status. * p < 0.1 ** p < 0.05 *** p < 0.01	

Table B6: Alternate version of Table B6, Panel B, where we use the full likert 
scale to generate the index outcomes. Variations in sample size are due to drop-off 
from telephone survey; order of survey modules was randomized. Outcomes are defined 
as described in Table 1, except that this version does not transform responses 
into binary indicators for above median response. All estimates include individual 
and household controls: age (above median dummy), education level (less than a 
high school degree), household size (number of members), 
		...
		* Run and store p-vals for FDR
		...
		foreach hte of global hte_var {
			...
			* create a var with a standardized name for HTE var so that estimates 
			...
			mat `hte'_pval_b1 = (`p_G1_2_likert_b1' \ `p_G1_3_likert_b1')
			mat `hte'_pval_b13 = (`p_G1_2_likert_b13' \ `p_G1_3_likert_b13')
...
* Write to latex
	* ALT TABLE B5
	* PANEL A - AGE
	esttab G1_2_likert_hte1  G1_3_likert_hte1 using ///
		"$output_rct/robustness/Alt_Table_B5_Panel_A_Likert.tex", ///
	...
	* PANEL B - EDU		 
	esttab G1_2_likert_hte2  G1_3_likert_hte2 using ///
		"$output_rct/robustness/Alt_Table_B5_Panel_B_Likert.tex", ///
		label se nogaps nobaselevels noobs ///
		keep(*treatment *hte) b(%9.3f) se(%9.3f) star(* 0.1 ** 0.05 *** 0.01) ///	
		nomtitles ///
		varlabels(1.treatment "$\beta\textsubscript{1}$: Treatment" ///
...
* ALT TABLE B6
		...
		* PANEL B - In LF at BL 		 
	esttab G1_2_likert_hte3  G1_3_likert_hte3 using ///
		"$output_rct/robustness/Alt_Table_B6_Panel_B_Likert.tex", ///
		label se nogaps nobaselevels noobs ///
		keep(*treatment *hte) b(%9.3f) se(%9.3f) star(* 0.1 ** 0.05 *** 0.01) ///	
		nomtitles ///
		varlabels(1.treatment "$\beta\textsubscript{1}$: Treatment" ///
