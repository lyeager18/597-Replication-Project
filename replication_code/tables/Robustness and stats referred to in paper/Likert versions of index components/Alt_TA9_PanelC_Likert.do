...
Purpose:	Robustness	- 	Alternate Table A9, Panel C; Permissions (leave house 
							and make purchase) and second order attitudes with 
							Lee bounds; Likert version
	
Table footnotes: Alternate version of Table A9, Panel C, where we use the full 
likert scale to generate the index outcome. Variations in sample size are due to 
drop-off from telephone survey; order of survey modules was randomized. Because 
our strata are small, Lee bounds are unstable with the strata and control variables 
in our preferred specification, so this table includes the main point estimate 
and the bounds estimated with no controls or fixed effects. SEs are clustered 
...
* Set global for table outcomes
	...
	* For Leebounds
	global 	permissions_2ndattitudes_lee G1_2_likert_lee G1_3_likert_lee ///
			ga2nd_fcom_likert_sw_lee ga2nd_allmen_likert_sw_lee
...
* Write to latex
	...
	"$output_rct/robustness/Alt_Table_A9_Panel_C_Likert.tex", ///
	...
	esttab	$permissions_2ndattitudes_lee using ///
		"$output_rct/robustness/Alt_Table_A9_Panel_C_Likert_Lee.tex", ///
